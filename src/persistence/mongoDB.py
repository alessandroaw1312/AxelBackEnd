...
def get_context_from_graph(graph_id):
    if graph_id:
        try:
            result = graphs_coll.find_one({"graph_id": str(graph_id)})
            if result:
                print(f"+++ GRAPH RETRIEVED SUCCESSFULLY +++")
                return "Documents Topic: " + result['topic'] + " Documents Summary:" + result['summary']
            else:
                print("+++ ERROR RETRIEVING GRAPH +++")
        except Exception as e:
            print("HEYYYY")
...
def serialize_mongo_document(graph_document):
    if '_id' in graph_document:
        del graph_document['_id']
    print("RETURNING GRAPH", graph_document)
...
def create_new_thread(user_id, graph_id):
    ...
    created_thread_document = {"thread_id": created_thread_id, "user_id": str(user_id), "graph_id": str(graph_id), "messages": []}
